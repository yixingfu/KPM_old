! Branched=Sun 25 Mar 2018 09:33:21 PM EDT from get_mu2d.f90
! Last Modified=Wed 28 Mar 2018 03:44:37 PM EDT
...
! IF inherit file, read psi0 and mu up to prevNc-1
        if (Inherit) then
                j0=prevNc
                read(94) mu2d(0:prevNc-1,0:prevNc-1)
        else
! If not inheriting
        call ResetRandSeed()
      ...
      ! now the central part: do the loops and find mu2d
        ...
        if (slowOPTCOND) then
!!!! SLOW VERSION BEGIN 
        write(*,*) "slow version not tested for inheriting files"
...
!!!! FAST VERSION BEGINS
        ...
        if (Inherit) then
                ...
                read(94) psi_p_out
                read(94) psi_pp_out
                read(94) psi_all_out(0:prevNc-1,N)
        else
                psi_pp_out = psi0_out
                psi_p_out = psi1_out
