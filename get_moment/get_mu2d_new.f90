...
! Last Modified=Thu 01 Feb 2018 09:46:51 AM EST
...
!<out|in>=<0|JHHHHJHHHHH|0>
      ...
      mu2d(0,1) = dot_product(psi0_out,psi_tmp)!<JJH>
      mu2d(1,0) = mu2d(0,1)

      call CSRmultVc16(N,NNZ,A,rp,col,psi0_out,psi1_out)
        !psi1_out=HJ|psi0>
      mu2d(1,1) = dot_product(psi1_out,psi_tmp)!<HJHJ>

      psi_pp_in = psi0!<psi0|
      psi_p_in  = psi1!<psi0|H
      ...
      enddo
                ...
                psi_pp_out = psi0_out!J|psi0>
                psi_p_out = psi1_out!HJ|psi0>
      ...
      do j=2,Nc-1
        call CSRmultVc16(N,NNZ,A,rp,col,psi_p_in,psi_tmp)
        psi_in = 2d0*psi_tmp-psi_pp_in!Tj(H)|>
        call CSRmultVc16(N,JNNZ,JA,Jrp,Jcol,psi_in,psi_tmp)
        !psi_tmp=JTj(H)|>
        psi_pp_in = psi_p_in
        psi_p_in  = psi_in
        ...
        call CSRmultVc16(N,NNZ,A,rp,col,psi_p_out,psi_tmp_out)
        psi_out = 2d0*psi_tmp_out-psi_pp_out!TjHJ|>
        psi_pp_out = psi_p_out
        psi_p_out  = psi_out
